...
const OnboardingView = ({ buttonLabel, disabled, onStart }: OnboardingViewProps) => (
  ...
  >
    <section
      className="w-full max-w-[26rem] rounded-[2rem] bg-white
        bg-[radial-gradient(circle_at_65%_0,_rgba(253,187,45,0.8)_0%,_transparent_55%)] p-5
        shadow-[0_24px_60px_-36px_rgba(90,40,7,0.45)] ring-1 ring-[#f1e8d5]"
    >
      ...
      <div className="space-y-7 px-1 pb-1">
        <div className="space-y-2 text-center">
          <h1 className="text-[1.45rem] leading-tight font-semibold text-[#202020] sm:text-[1.6rem]">
            하루를 채우는 작은 기록
          </h1>
          <p className="text-[0.94rem] leading-relaxed text-[rgba(32,32,32,.6)] sm:text-[0.98rem]">
            오늘의 감정과 순간을 담아내며,
            <br />
        ...
        <button
          ...
          onClick={onStart}
          disabled={disabled}
          className="w-full rounded-[0.95rem] bg-[rgb(231,174,67)] px-4 py-3 text-sm font-medium
            text-white transition hover:bg-[#254777] disabled:cursor-not-allowed
            disabled:bg-[#8ea0ba]"
        >
          {buttonLabel}
        </button>
